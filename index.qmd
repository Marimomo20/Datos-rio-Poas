...
# Convertir Vitesse a numérico
datos <- datos %>% mutate(Vitesse = as.numeric(gsub(",", ".", Vitesse)))

# Calcular el rango intercuartílico (IQR)
Q1 <- quantile(datos$Vitesse, 0.25, na.rm = TRUE)
Q3 <- quantile(datos$Vitesse, 0.75, na.rm = TRUE)
IQR_value <- Q3 - Q1
...
# Crear una nueva columna indicando si el valor es outlier
datos <- datos %>%
  mutate(Outlier = ifelse(Vitesse < limite_inferior | Vitesse > limite_superior, "Sí", "No"))

# Filtrar datos extremos para mejorar la escala del gráfico
datos_filtrados <- datos %>% filter(Vitesse <- 7000 | Vitesse >- 200)
```
...
# Gráfico boxplot interactivo de Vitesse mejorado
fig_vitesse <- plot_ly(
...
# Crear la ventana de 40 minutos (ajustable según el intervalo de los datos)
ventana <- 40  # En minutos

# Calcular el promedio móvil usando una ventana de 40 minutos
datos$Vitesse_ma <- rollapply(datos$Vitesse, width = ventana, FUN = mean, fill = NA, align = "center")

# Calcular la diferencia porcentual
datos$Diferencia <- abs(datos$Vitesse - datos$Vitesse_ma) / datos$Vitesse_ma

# Identificar valores que difieran en más del 30%
datos$Vitesse_corregida <- ifelse(datos$Diferencia > 0.30, NA, datos$Vitesse)

# Visualizar los datos corregidos
head(datos)
```
...
# Crear gráfico interactivo con puntos en lugar de líneas
fig <- plot_ly(datos, x = ~DateTime) %>%
  add_trace(y = ~Vitesse, name = "Vitesse Original", mode = 'markers', marker = list(color = 'red', size = 5, opacity = 0.5)) %>%
  add_trace(y = ~Vitesse_corregida, name = "Vitesse Corregida", mode = 'markers', marker = list(color = 'blue', size = 5)) %>%
  layout(title = "Evolución de Vitesse en el Tiempo",
         xaxis = list(title = "Tiempo"),
         yaxis = list(title = "Vitesse"),
         hovermode = "x unified") 
